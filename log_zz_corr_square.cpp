...
/*
 * This file contains functions related to log_zz_corr, which is the log-average of end-to-end zz correlation square
...
/*
 * Initialize log_zz_corr_square in model_data_. The outer index is for J, and the inner index is for realization
 */
void DisorderModelTransition::Log_ZZ_corr_square_init_(AllPara const & parameters) {
    const int J_N = parameters.floquet.J_N; // Number of points for J
    const int num_realization = parameters.generic.num_realizations;

    model_data_.log_zz_corr_square.resize(J_N);

    for (int i=0; i<J_N; i++){
        model_data_.log_zz_corr_square[i].resize(num_realization);

        for (int j=0; j<num_realization;j++) model_data_.log_zz_corr_square[i][j] = 0;
    }
...
/*
 * Compute log_zz_corr_square given J index and realization index
 */
void DisorderModelTransition::Log_ZZ_corr_square_compute_(AllPara const & parameters,
    ...
    if (local_info.evec_type_real){
        ...
        for (int i=0; i<evec_basic.size(); i++){
            ...
            zz_square += log( (left_right_temp - left_temp*right_temp ) * (left_right_temp - left_temp*right_temp ) );
            if (debug){
    ...
    model_data_.log_zz_corr_square[local_info.J_index][local_info.realization_index] += zz_square / double(dim);
    if (debug){
...
/*
 * Output averages of mean end-to-end log zz correlation squares (over all eigenstates for each realization) and their
 * standard deviations among all realizations for each J
 */
void DisorderModelTransition::Log_ZZ_corr_square_out_(AllPara const & parameters, const string& name) {
    ...
    if (model_data_.log_zz_corr_square.size() != J_N){
        cout << "Not enough number of J for zz correlation square." << endl;
        cout << "Expected Number: " << J_N << endl;
        cout << "Actual number: " << model_data_.log_zz_corr_square.size() << endl;
        abort();
    }

    for (int i=0; i< J_N; i++){
        if (model_data_.log_zz_corr_square[i].size() != num_realizations){
            ...
            cout << "Actual Number: " << model_data_.log_zz_corr_square[i].size() << endl;
            abort();
        }
    }
    ...
    << ",J_max=" << J_max << ",log_zz_corr_square";
    ...
    for (int i=0; i<J_N;i++){
        ...
        generic_mean_sd(model_data_.log_zz_corr_square[i], mean, sd);
