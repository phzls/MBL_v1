    ...
    for(int i=0; i<dim_; i++){
        ...
        for(int j=0; j<dim_; j++){
            ...
            for(int k=0; k<size_; k++){
                if(spin1[k] == spin2[k]) prod *= cos( tau_*g_*(J_+sigma_*gaussian_[k])/2.0 );
                else prod *= complex<double>( 0, -sin( tau_*g_*(J_+sigma_*gaussian_[k])/2.0 ) );
            }
...
void FloEvolXXZGaussianRandomShiftReal::Evol_Z_Construct_(MatrixXcd & evol_z) {
    ...
    for (int i=0; i<dim_;i++){
        ...
        for (int j=0; j<size_;j++){
            value += h_*(J_+sigma_*gaussian_[j])*(2*state[j]-1);
...
void FloEvolXXZGaussianRandomShiftReal::Get_Ham(MatrixXcd& ham, string basis, string s){
    ...
    for(int i=0; i<dim_; i++){
        ...
        for (int j=0; j<size_;j++){
            z_value += h_*(J_+sigma_*gaussian_[j])*(2*state[j]-1);
            ...
            ham(x_state, i) += g_*(J_+sigma_*gaussian_[j]);
        }
