...
void reduced_density_left_2(const VectorXd& state_basic, int size, int left_size,
                            MatrixXd& reduced_density){
    const int right_size = size - left_size; // Length of right part of the chain
    const int left_dim = 1 << left_size; // Total dimension of the left part Hiblert space
    const int right_dim = 1 << right_size; // Total dimension of the right part space
    ...
    reduced_density = MatrixXd::Zero(left_dim, left_dim);
    ...
    for(int i=0; i<left_dim; i++)
    {
        for(int j=0; j<=i; j++)
        {
            for(int k=0; k<right_dim; k++)
            {
                reduced_density(j,i) += state_basic(j*right_dim + k) * state_basic(i*right_dim + k) ;
            }
            if(i != j) reduced_density(i,j) = reduced_density(j,i);
        }
    }
}
...
void reduced_density_left_2(const VectorXd& state_basic, int size, int left_size,
    ...
    for(int i=0; i<left_dim; i++)
    {
        for(int j=0; j<=i; j++)
        {
            for(int k=0; k<right_dim; k++)
            {
                reduced_density(j,i) += complex<double>(
                        state_basic(j*right_dim + k) * state_basic(i*right_dim + k), 0 ) ;
            }
            if(i != j) reduced_density(i,j) = reduced_density(j,i);
        }
    }
