...
// For XXZ general random field shift real floquet operator
void XXZ_General_Random_Field_Shift_Real_Flo_Para(AllPara& parameters, string count){
    string filename = "xxz_general_random_field_shift_real_flo_" + count;
    vector<vector<string> > content;
    string keyword = "J";
...
void Linked_Cluster_Para(AllPara& parameters, string count){
    ...
    keyword = "order";
    para_get(filename, content, keyword, parameters.linked_cluster_para.order);

    vector<string> linked_cluster_map;
    vector<string>::iterator it;

    linked_cluster_map.push_back("Average_Entropy");
