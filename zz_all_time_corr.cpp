//
// Created by Liangsheng Zhang on 5/5/15.
//

/*
 * This file contains functions related to zz_all_time_corr, which is the zz time correlation for all
...
/*
 * Initialize zz_all_time_corr in model_data_. The outer index is for J; middle index for for all possible
 * configurations, and the inner index is for realization
 */
void DisorderModelTransition::ZZ_all_time_corr_init_(AllPara const & parameters) {
    ...
    model_data_.zz_all_time_corr.resize(J_N);

    for (int i=0; i<J_N; i++){
        model_data_.zz_all_time_corr[i].resize(size/2);

        for (int j=0; j<size/2;j++){
            model_data_.zz_all_time_corr[i][j].resize(num_realization);

            for (int k=0; k<num_realization; k++){
                model_data_.zz_all_time_corr[i][j][k] = 0;
            }
        }
    }
...
/*
 * Compute zz_all_time_corr given realization index
 */
void DisorderModelTransition::ZZ_all_time_corr_compute_(AllPara const & parameters,
                                                          const EvolOP* model,
    ...
    if (local_info.evec_type_real){
        ...
        for (int i=0; i<size/2;i++){
            ...
            model_data_.zz_all_time_corr[local_info.J_index][i][local_info.realization_index]
                    += zz_time_corr;
 ...
 */
void DisorderModelTransition::ZZ_all_time_corr_out_(AllPara const & parameters, const string& name) {
    ...
    if (model_data_.zz_all_time_corr.size() != J_N){
        cout << "Different number of J for zz all correlation square." << endl;
        cout << "Expected Number: " << J_N << endl;
        cout << "Actual number: " << model_data_.zz_all_time_corr.size() << endl;
        abort();
    }

    for (int i=0; i<J_N; i++){
        if (model_data_.zz_all_time_corr[i].size() != size/2){
            cout << "Different number of configurations at " << i <<"th J for zz all correlation square." << endl;
            cout << "Expected Number: " << size/2 << endl;
            cout << "Actual Number: " << model_data_.zz_all_time_corr[i].size() << endl;
            abort();
        }

        for (int j=0; j<size/2;j++){
            if (model_data_.zz_all_time_corr[i][j].size() != num_realizations){
                ...
                cout << "Actual Number: " << model_data_.zz_all_time_corr[i][j].size() << endl;
                abort();
            }
        }
    }
    ...
    for (int i=0; i<J_N;i++){
        ...
        for (int j=0; j<model_data_.zz_all_time_corr[i].size(); j++){

            generic_mean_sd(model_data_.zz_all_time_corr[i][j], mean, sd);
