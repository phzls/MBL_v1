//
// Created by Liangsheng Zhang on 5/4/15.
//

/*
 * This file contains functions related to log_zz_all_corr_square, which is the log of zz correlation square average
 * over different eigenstates for all configurations symmetric about spin chain center.
 */
...
/*
 * Initialize log_zz_all_corr_square in model_data_. The outer index is for J; middle index for for all possible
 * configurations, and the inner index is for realization
 */
void DisorderModelTransition::Log_ZZ_all_corr_square_init_(AllPara const & parameters) {
    ...
    model_data_.log_zz_all_corr_square.resize(J_N);

    for (int i=0; i<J_N; i++){
        model_data_.log_zz_all_corr_square[i].resize(size/2);

        for (int j=0; j<size/2;j++){
            model_data_.log_zz_all_corr_square[i][j].resize(num_realization);

            for (int k=0; k<num_realization; k++){
                model_data_.log_zz_all_corr_square[i][j][k] = 0;
            }
        }
    }
...
/*
 * Compute log_zz_all_corr_square given realization index
 */
void DisorderModelTransition::Log_ZZ_all_corr_square_compute_(AllPara const & parameters,
                                                          const EvolOP* model,
    ...
    if (local_info.evec_type_real){
        ...
        for (int i=0; i<size/2;i++){
            ...
            for (int j=0; j<evec_basic.size();j++){
                ...
                zz_square += log( (left_right_temp - left_temp*right_temp ) *
                                          (left_right_temp - left_temp*right_temp ) );
                if (debug){
            ...
            model_data_.log_zz_all_corr_square[local_info.J_index][i][local_info.realization_index]
 ...
 */
void DisorderModelTransition::Log_ZZ_all_corr_square_out_(AllPara const & parameters, const string& name) {
    ...
    if (model_data_.log_zz_all_corr_square.size() != J_N){
        cout << "Different number of J for zz all correlation square." << endl;
        cout << "Expected Number: " << J_N << endl;
        cout << "Actual number: " << model_data_.log_zz_all_corr_square.size() << endl;
        abort();
    }

    for (int i=0; i<J_N; i++){
        if (model_data_.log_zz_all_corr_square[i].size() != size/2){
            cout << "Different number of configurations at " << i <<"th J for zz all correlation square." << endl;
            cout << "Expected Number: " << size/2 << endl;
            cout << "Actual Number: " << model_data_.log_zz_all_corr_square[i].size() << endl;
            abort();
        }

        for (int j=0; j<size/2;j++){
            if (model_data_.log_zz_all_corr_square[i][j].size() != num_realizations){
                ...
                cout << "Actual Number: " << model_data_.log_zz_all_corr_square[i][j].size() << endl;
                abort();
            }
        }
    }
    ...
    for (int i=0; i<J_N;i++){
        ...
        for (int j=0; j<model_data_.log_zz_all_corr_square[i].size(); j++){

            generic_mean_sd(model_data_.log_zz_all_corr_square[i][j], mean, sd);
